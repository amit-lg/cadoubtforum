...
const Question = ({
  ...
  const [likeValue, setLikeValue] = useState(question?._count?.likes);
    ...
    <div>
      <div className="px-1 flex items-center gap-3">
        <div onClick={goToQuestion} className="w-full">
          <Card className="relative flex bg-white flex-col my-3 cursor-pointer min-h-24 md:min-h-32 rounded-md px-1 py-1">
            <div
              className={`${
                size === "large" ? "md:invisible flex" : "flex"
              } justify-end pb-1`}
            >
              <div className="flex text-gray-400 mx-3 items-center gap-2">
                ...
                <EachActionButton
                  selected={question?.likes?.length !== 0}
                  Icon={<MdThumbUp/>}
                  value={question?._count?.likes}
                />
            ...
            <div className="flex justify-end lg:justify-between items-center pt-2 px-1">
              <div className="hidden gap-3 lg:flex text-sm font-semibold text-gray-400 overflow-hidden h-5 w-2/3 whitespace-nowrap mx-3">
                <div className="flex">
                  {
                    question?.fallnumber?.Points[0]?.topics
                      ?.Subjects?.name
                  }
                </div>
                <div className="flex">
            ...
            <div className="px-3 flex gap-2">
              {size === "large" && question?.attachments?.length !== 0 &&
                question?.attachments?.map((attachment) => (
                  <div onClick={() => openPopUp(attachment?.ImagePath)} key={attachment?.id} className="h-12 w-12 rounded-md">
                    <img
            ...
            <div className="absolute -top-8 flex gap-3">
              <Avatar className="mt-3" user={user} />
              <div className="flex mt-3 text-orange-300 font-semibold items-center gap-1">
        ...
        {size === "large" && (
          <div className="md:flex hiddenflex flex-col gap-3 content-between">
      ...
      >
        ...
        >
          {showReplyBox && (
            ...
            >
              <textarea
                className={`w-full border-none outline-none rouded-md p-2 resize-none ${replyError ? "border-red-500" : ""}`}
                rows={6}
                value={reply}
