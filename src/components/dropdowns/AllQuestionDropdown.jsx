...
const AllQuestionDropdowns = ({ pointError, type }) => {
  const {
    subjects,
    topics,
    points,
  ...
  const [isPointEmpty, setIsPointEmpty] = useState(false);
  ...
  const fetchDropdownData = async (data) => {
    ...
    if (response.status === 200) {
      ...
      if (Array.isArray(response.data)) {
        ...
        setIsTopicEmpty(isEmptyTopic);
        setIsPointEmpty(isEmptyPoint);
        dispatch(setPointName(""));
        dispatch(setPointsValue(""));
      } else {
        let tempPoints = [];
  ...
  return (
    <div className={`mt-4 mx-auto gap-2 md:gap-4 grid grid-cols-12`}>
      <SubjectDropdown
        data={subjects}
        id={"subject-dropdown"}
        value={subjectValue}
...
export const SubjectDropdown = ({ data, id, display }) => {
  ...
  return (
    <div
      id={id}
      ref={impactRef}
      className={`relative h-[50px] md:col-span-4 col-span-12 `}
    >
      <div
        onClick={toggleMenu}
...
export const PointDropdown = ({ data, id, disabled, display, pointError }) => {
  const [showMenu, setShowMenu] = useState(false);
  ...
  return (
    <div
      id={id}
      ref={targetRef}
      className={`relative h-[50px] col-span-4 md:col-span-4"
      }`}
    >
      <div
        onClick={disabled ? null : toggleMenu}
