...
import { setSubjects, setTopics, setPoints, setTopicValue, setTopicName, setPointsValue, setPointName, setSubjectName, setSubjectVlue } from "../../redux/reducers/pinnedQuestionsReducer";
import useDidMountEffect from "../../hooks/useUpdateEffect";
// import { dropdownData } from "../mocks/dummy";

const PinnedQuestionDropdowns = ({
    pointError,
    type,
}) => {
    const { subjects, topics, points, subjectName, topicName, pointsName, subjectValue, topicValue, pointsValue } = useSelector((state) => state.pinned);
    ...
    useEffect(() => {
        if((subjectValue === "" || subjectValue === "empty") && (topicValue === "" || topicValue === "empty") && (pointsValue === "" || pointsValue === "empty")){
            fetchDropdownData({});
        }
    // eslint-disable-next-line react-hooks/exhaustive-deps
    },[]);
    ...
    return (
        <div className="mt-4 w-full gap-2 md:gap-4 grid grid-cols-12 justify-between">
            <SubjectDropdown
                data={subjects}
                id={"subject-dropdown"}
                value={subjectValue}
...
export default PinnedQuestionDropdowns;

PinnedQuestionDropdowns.propTypes = {
...
export const SubjectDropdown = ({
    ...
    return (
        <div
            id={id}
            ref={impactRef}
            className={`relative h-[50px] col-span-12 ${isTopicEmpty ? "md:col-span-6" : "md:col-span-4"
                }`}
        >
            <div
                onClick={toggleMenu}
...
export const PointDropdown = ({
    ...
    return (
        ...
        >
            <div
                onClick={disabled ? null : toggleMenu}
                className={`${pointError ? "border-2 border-red-500" : ""
